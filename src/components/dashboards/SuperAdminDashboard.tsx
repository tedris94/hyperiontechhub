...
import { Users, Settings, ShieldCheck, Activity, DollarSign, MessageSquare, Mail, Phone, Clock, CheckCircle, X, Send, AlertCircle, Video, UserPlus, Calendar } from 'lucide-react';
...
export function SuperAdminDashboard() {
  ...
  useEffect(() => {
    ...
    const loadMetrics = async () => {
      ...
      const storedRevenue = await getStoredRevenueTotal();
      setRevenueTotal(
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      const reply = {
        ...
        sentBy: user?.fullName || 'Super Admin',
      };

      await addReply(selectedSubmission.id, replyMessage, user?.fullName || 'Super Admin');
  ...
  const recentUsers = [
    { id: '1234', name: 'John Doe', email: 'john@example.com', role: 'Student', joined: '2 hours ago', status: 'Active' },
    { id: '1233', name: 'Sarah Smith', email: 'sarah@company.com', role: 'Client', joined: '5 hours ago', status: 'Active' },
    { id: '1232', name: 'Mike Johnson', email: 'mike@edu.com', role: 'Instructor', joined: '1 day ago', status: 'Active' },
  ];
                        ...
                        onClick={async () => {
                          if (selectedConsultant) {
                            ...
                            if (consultant) {
                              ...
                              const meetLink = await generateGoogleMeetLink(selectedConsultation.id);
                ...
                {selectedSubmission.replies && selectedSubmission.replies.length > 0 && (
                  <div className="pt-4 border-t">
                    ...
                    <div className="space-y-3">
                      {selectedSubmission.replies.map((reply, replyIndex) => (
                        <div key={replyIndex} className="bg-blue-50 p-3 rounded-lg border border-blue-200">
